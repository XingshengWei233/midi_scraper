...
SAVE_DIR = '/media/xingshengwei/Seagate Portable Drive/scrape/EOP_data_mp3'
...
def scrape_one(url_tuple):
    ...
    for item in soup.find_all('head'): 
        song_name = item.title.text
        #print('song_name:', song_name)
    for item in soup.find_all('audio'):
        #print('item:', item) 
        dl_page = item["src"]

        #print(item["src"])
        file_name = song_name.replace('/','-') + '.mp3'
...
def get_num_score_from_EOP(begin, end):
    ...
    for i in range(begin,end): #14212
        url = f"https://www.everyonepiano.cn/Mp3-{i}.html"
        url_tuple = (i,url)
...
if __name__ == '__main__':
    ...
    #mp3 = requests.get(url='https://play.everyonepiano.cn/015/0014218-172n17nt.mp3',headers=HEADERS)
    #with open(os.path.join(SAVE_DIR,'test.mp3'), 'wb') as f:
    #    f.write(mp3.content)
