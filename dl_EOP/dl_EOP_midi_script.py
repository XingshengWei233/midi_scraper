...
SAVE_DIR = '/media/xingshengwei/Seagate Portable Drive/scrape/EOP_data_midi_2'
...
def test_EOP_cookie():
    ...
    # 'id': 'search-result'
    for item in soup.find_all('p', {'class': 'UserName'}):
        print('username: ', item.text)


def scrape_one(url_tuple):
    '''process of downloading one file, to be called by pool'''
    ...
    # make a log file to remember downloaded file, so the repeated file will be skipped
